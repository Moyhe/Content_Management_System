...
<div id="collapseOne<?php echo $row['id'];?>" class="accordion-collapse collapse show" aria-labelledby="headingOne" data-bs-parent="#accordionExample">
    ...
    <li class="nav-item">
    <a class="nav-link" aria-current="page" href="edit_menu.php?page=<?php echo mysqli_real_escape_string($conn, $row1['id']);?>"><?php echo mysqli_real_escape_string($conn, $row1['page_name']);?></a>
  </li>
    </ul> 
...
<?php
  ...
  if (mysqli_num_rows($result) > 0) {
    ...
    <input type="text" class="form-control" value="<?php echo $row['item_name'] ?>" id="menu" placeholder="Menu Name" name="menu"  aria-describedby="emailHelp">
  </div>
...
?>
    <option value="<?php  echo $row[$i]; ?>"><?php echo mysqli_real_escape_string($conn, $i); ?></option>
<?php
}
